...
import {
  getPoliticsPageSections,
  putPoliticsPageSections,
  type IPoliticsPageSectionItem,
  type PoliticsPageSectionType,
...
/** Placement / Politics boards share this page name when syncing slots. */
const POLITICS_EDITOR_PAGE_NAME = 'politics'
...
const SECTION_TYPES: PoliticsPageSectionType[] = [
  'hero',
  'top_stories',
  'live',
  'more_top_stories',
...
const DEFAULT_LABEL_BY_TYPE: Record<PoliticsPageSectionType, string> = {
  hero: 'Politics',
  top_stories: 'Top Stories',
  live: 'Live',
  more_top_stories: 'Congress',
  spotlight: 'Elections',
  rail: 'White House',
  category: '',
...
const CANONICAL_SLUG_BY_TYPE: Partial<Record<PoliticsPageSectionType, string>> = {
  hero: 'hero',
  top_stories: 'us-featured',
  live: 'health',
}

const PREFERRED_SLUG_PREFIX_BY_TYPE: Partial<Record<PoliticsPageSectionType, string>> = {
  more_top_stories: 'more-top-stories',
  spotlight: 'politics-spotlight',
  rail: 'editorial-rail',
  ribbon_ad: 'ad-ribbon',
}
...
interface IEditableSectionRow {
  key: string
  sectionType: PoliticsPageSectionType
  label: string
  slug: string
}
 ...
 *
 * @param items Saved Politics-page section items.
 * @returns Editable row models with stable keys.
 */
function toEditableRows(items: IPoliticsPageSectionItem[]): IEditableSectionRow[] {
...
/**
 * Region code for Politics section scope (country, state, county, or town).
 *
 ...
 */
function politicsSectionsRegionCode(
  marketCode: string,
...
/**
 * Build the editor scope used to invalidate Placement after a Politics save.
 *
 * @param marketCode Active market code.
 * @param localityId US state or PR town short code.
 * @param countyId Optional Florida county slug.
 * @returns Scope matching the Politics board that was just written.
 */
function politicsEditorScope(
  marketCode: string,
  localityId: string | null,
  countyId: string | null,
): IEditorScope {
  return {
    ...
    townId: localityId,
    countyId,
    pageName: POLITICS_EDITOR_PAGE_NAME,
  }
}

/**
 * Admin editor for the ordered Politics page section list of one geo scope.
 *
 * @returns Politics-page sections administration UI.
 */
export function PoliticsSectionsEditor(): JSX.Element {
  ...
  const regionCode = politicsSectionsRegionCode(marketCode, localityId, countyId)
  const scopeKey = `${marketCode}:${regionCode}`

  const loading = useGenerationLoading(scopeKey, async (isCurrent) => {
    try {
      const data = await getPoliticsPageSections(marketCode, regionCode)
      if (!isCurrent()) {
        return
      }
      ...
      const message = error instanceof Error ? error.message : t('PoliticsPage.loadFailed')
  ...
  async function persistRows(nextRows: IEditableSectionRow[] = rows): Promise<void> {
    ...
    setSaving(true)
    try {
      const data = await putPoliticsPageSections(marketCode, items, regionCode, toApiAdRows(adRows))
      setRows(toEditableRows(data.items))
      setAdRows(toEditableAdRows(data.ads))
      notifyEditorialPreviewStale(politicsEditorScope(marketCode, localityId, countyId))
      pushToast(t('PoliticsPage.saveSuccess'), 'success')
    } catch (error) {
      const message = error instanceof Error ? error.message : t('PoliticsPage.saveFailed')
      pushToast(message, 'error')
    } finally {
      setSaving(false)
    }
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`PoliticsPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        <p className="text-sm text-neutral-600">{t('PoliticsPage.loading')}</p>
      ) : (
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('PoliticsPage.loading')}</p>
          ) : null}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('PoliticsPage.removeConfirm', { label: pendingRemoveLabel })}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {saving ? t('PoliticsPage.saving') : t('PoliticsPage.save')}
        </button>
      </div>
    </div>
  )
...
/**
 * Editable ordered list of Politics-page sections.
 */
function PoliticsRowsEditor({
  rows,
  onChange,
  onRemove,
}: {
  ...
  function addSection(sectionType: PoliticsPageSectionType): void {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-neutral-600">{t('PoliticsPage.listHint')}</p>
      <ul className="space-y-2">
        {rows.map((row, index) => (
          <SortableConfigRow
            ...
            dragHandleLabel={t('PoliticsPage.dragHandle')}
      ...
      {preview && previewRow ? (
        <SortableDragPreview preview={preview}>
          <PoliticsRowPreview row={previewRow} index={preview.index} />
        </SortableDragPreview>
      ) : null}
      <div className="flex flex-wrap gap-2">
        {SECTION_TYPES.map((sectionType) => {
          ...
          return (
            ...
            >
              {t(`PoliticsPage.addType.${sectionType}`)}
            </button>
          )
        })}
      </div>
...
interface IPoliticsRowFieldsProps {
...
/**
 * Inline editors and move/remove controls for one politics section row.
 */
function PoliticsRowFields(props: IPoliticsRowFieldsProps): JSX.Element {
  ...
  return (
    <>
      ...
      <span className="rounded bg-neutral-100 px-2 py-1 text-xs font-semibold uppercase tracking-wide text-neutral-700">
        {t(`PoliticsPage.types.${row.sectionType}`)}
      </span>
      <input
        value={row.label}
        onChange={(event) =>
        ...
        placeholder={t('PoliticsPage.labelPlaceholder')}
        className={`${INPUT_CLASS} min-w-[12rem] flex-1`}
        aria-label={t('PoliticsPage.labelPlaceholder')}
      />
...
/**
 * Compact floating preview content for a dragged politics row.
 */
function PoliticsRowPreview({
  row,
  index,
}: {
  row: IEditableSectionRow
  ...
  return (
    <>
      ...
      <span className="min-w-[12rem] flex-1 truncate text-sm font-medium text-neutral-900">
        {row.label || t('PoliticsPage.labelPlaceholder')}
      </span>
    </>
  )
}
