...
function PoolPrimaryFilters(props: IPoolPrimaryFiltersProps): JSX.Element {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 md:items-end lg:grid-cols-4">
      ...
      {governmentSelected ? (
        <FilterField label={t('editor.pool.filterBar.governmentTopicLabel')}>
          <select
            value={filters.sportCategoryId}
            disabled={disabled || governmentSectionsQuery.isLoading}
            onChange={(event) => onUpdate({ sportCategoryId: event.target.value })}
            className={filterControlClass(disabled)}
          >
      ...
      {healthSelected ? (
        <FilterField label={t('editor.pool.filterBar.healthTopicLabel')}>
          <select
            value={filters.sportCategoryId}
            disabled={disabled || healthSectionsQuery.isLoading}
            onChange={(event) => onUpdate({ sportCategoryId: event.target.value })}
            className={filterControlClass(disabled)}
          >
            <option value="">{t('editor.pool.filterBar.healthTopicAll')}</option>
            {healthChildren.map((category) => (
              <option key={category.id} value={category.id}>
                {category.name}
              </option>
            ))}
