...
import {
  getGovernmentPageSections,
  putGovernmentPageSections,
  type IGovernmentPageSectionItem,
  type GovernmentPageSectionType,
...
/** Placement / government boards share this page name when syncing slots. */
const GOVERNMENT_EDITOR_PAGE_NAME = 'government'
...
const SECTION_TYPES: GovernmentPageSectionType[] = [
  'hero',
  'top_stories',
  'live',
  'world',
...
const DEFAULT_LABEL_BY_TYPE: Record<GovernmentPageSectionType, string> = {
  hero: 'Government',
...
const CANONICAL_SLUG_BY_TYPE: Partial<Record<GovernmentPageSectionType, string>> = {
  hero: 'hero',
...
const PREFERRED_SLUG_PREFIX_BY_TYPE: Partial<Record<GovernmentPageSectionType, string>> = {
  ribbon_ad: 'ad-ribbon',
  topic: 'topic',
}
...
interface IEditableSectionRow {
  key: string
  sectionType: GovernmentPageSectionType
  label: string
  slug: string
}
 ...
 *
 * @param items Saved government section items.
 * @returns Editable row models with stable keys.
 */
function toEditableRows(items: IGovernmentPageSectionItem[]): IEditableSectionRow[] {
...
/**
 * Region code for government section scope (country, state, county, or town).
 *
 ...
 */
function governmentSectionsRegionCode(
  marketCode: string,
...
/**
 * Build the editor scope used to invalidate Placement after a government save.
 *
 * @param marketCode Active market code.
 * @param localityId US state or PR town short code.
 * @param countyId Optional Florida county slug.
 * @returns Scope matching the government board that was just written.
 */
function governmentEditorScope(
  marketCode: string,
  localityId: string | null,
  countyId: string | null,
): IEditorScope {
...
/**
 * Admin editor for the ordered government page section list of one geo scope.
 *
 * @returns Government sections administration UI.
 */
export function GovernmentSectionsEditor(): JSX.Element {
  ...
  const regionCode = governmentSectionsRegionCode(marketCode, localityId, countyId)
  const scopeKey = `${marketCode}:${regionCode}`

  const loading = useGenerationLoading(scopeKey, async (isCurrent) => {
    try {
      const data = await getGovernmentPageSections(marketCode, regionCode)
      if (!isCurrent()) {
        return
      }
      ...
      const message = error instanceof Error ? error.message : t('governmentPage.loadFailed')
  ...
  async function persistRows(nextRows: IEditableSectionRow[] = rows): Promise<void> {
    ...
    setSaving(true)
    try {
      const data = await putGovernmentPageSections(marketCode, items, regionCode, toApiAdRows(adRows))
      setRows(toEditableRows(data.items))
      setAdRows(toEditableAdRows(data.ads))
      notifyEditorialPreviewStale(governmentEditorScope(marketCode, localityId, countyId))
      pushToast(t('governmentPage.saveSuccess'), 'success')
    } catch (error) {
      const message = error instanceof Error ? error.message : t('governmentPage.saveFailed')
      pushToast(message, 'error')
    } finally {
      setSaving(false)
    }
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`governmentPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        ...
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('governmentPage.loading')}</p>
          ) : null}
          <div className={loading ? 'pointer-events-none opacity-60' : undefined}>
            <GovernmentRowsEditor
              rows={rows}
              onChange={setRows}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('governmentPage.removeConfirm', { label: pendingRemoveLabel })}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {saving ? t('governmentPage.saving') : t('governmentPage.save')}
        </button>
      </div>
    </div>
  )
...
/**
 * Editable ordered list of government page sections (hero, bands, government).
 */
function GovernmentRowsEditor({
  rows,
  onChange,
  onRemove,
}: {
  ...
  function addSection(sectionType: GovernmentPageSectionType): void {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-neutral-600">{t('governmentPage.listHint')}</p>
      <ul className="space-y-2">
        {rows.map((row, index) => (
          <SortableConfigRow
            ...
            dragHandleLabel={t('governmentPage.dragHandle')}
      ...
      {preview && previewRow ? (
        <SortableDragPreview preview={preview}>
          <GovernmentRowPreview row={previewRow} index={preview.index} />
        </SortableDragPreview>
      ) : null}
      <div className="flex flex-wrap gap-2">
        {SECTION_TYPES.map((sectionType) => {
          ...
          return (
            ...
            >
              {t(`governmentPage.addType.${sectionType}`)}
            </button>
          )
        })}
      </div>
...
interface IGovernmentRowFieldsProps {
...
/**
 * Inline editors and move/remove controls for one government section row.
 */
function GovernmentRowFields(props: IGovernmentRowFieldsProps): JSX.Element {
  ...
  return (
    <>
      ...
      <span className="rounded bg-neutral-100 px-2 py-1 text-xs font-semibold uppercase tracking-wide text-neutral-700">
        {t(`governmentPage.types.${row.sectionType}`)}
      </span>
      <input
        value={row.label}
        onChange={(event) =>
        ...
        placeholder={t('governmentPage.labelPlaceholder')}
        className={`${INPUT_CLASS} min-w-[12rem] flex-1`}
        aria-label={t('governmentPage.labelPlaceholder')}
      />
...
/**
 * Compact floating preview content for a dragged government row.
 */
function GovernmentRowPreview({
  row,
  index,
}: {
  row: IEditableSectionRow
  ...
  return (
    <>
      ...
      <span className="min-w-[12rem] flex-1 truncate text-sm font-medium text-neutral-900">
        {row.label || t('governmentPage.labelPlaceholder')}
      </span>
    </>
  )
}
