...
export function WorldSectionsEditor(): JSX.Element {
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`worldPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        ...
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('worldPage.loading')}</p>
          ) : null}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('worldPage.removeConfirm', { label: pendingRemoveLabel })}
