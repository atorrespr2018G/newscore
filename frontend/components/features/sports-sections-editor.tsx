...
export function SportsSectionsEditor(): JSX.Element {
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`sportsPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        ...
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('sportsPage.loading')}</p>
          ) : null}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('sportsPage.removeConfirm', { label: pendingRemoveLabel })}
