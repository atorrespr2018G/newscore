...
import {
  getBusinessPageSections,
  putBusinessPageSections,
  type IBusinessPageSectionItem,
  type BusinessPageSectionType,
...
/** Placement / business boards share this page name when syncing slots. */
const BUSINESS_EDITOR_PAGE_NAME = 'business'
...
const SECTION_TYPES: BusinessPageSectionType[] = [
  'hero',
  'top_stories',
  'live',
  'business',
...
const DEFAULT_LABEL_BY_TYPE: Record<BusinessPageSectionType, string> = {
  hero: 'Business',
...
const CANONICAL_SLUG_BY_TYPE: Partial<Record<BusinessPageSectionType, string>> = {
  hero: 'hero',
  top_stories: 'us-featured',
  live: 'health',
}

const PREFERRED_SLUG_PREFIX_BY_TYPE: Partial<Record<BusinessPageSectionType, string>> = {
  ribbon_ad: 'ad-ribbon',
  business: 'topic',
}
...
interface IEditableSectionRow {
  key: string
  sectionType: BusinessPageSectionType
  label: string
  slug: string
}
 ...
 *
 * @param items Saved business section items.
 * @returns Editable row models with stable keys.
 */
function toEditableRows(items: IBusinessPageSectionItem[]): IEditableSectionRow[] {
...
/**
 * Region code for business section scope (country, state, county, or town).
 *
 ...
 */
function businessSectionsRegionCode(
  marketCode: string,
...
/**
 * Build the editor scope used to invalidate Placement after a business save.
 *
 * @param marketCode Active market code.
 * @param localityId US state or PR town short code.
 * @param countyId Optional Florida county slug.
 * @returns Scope matching the business board that was just written.
 */
function businessEditorScope(
  marketCode: string,
  localityId: string | null,
  countyId: string | null,
): IEditorScope {
  return {
    ...
    townId: localityId,
    countyId,
    pageName: BUSINESS_EDITOR_PAGE_NAME,
  }
}

/**
 * Admin editor for the ordered business page section list of one geo scope.
 *
 * @returns Business sections administration UI.
 */
export function BusinessSectionsEditor(): JSX.Element {
  ...
  const regionCode = businessSectionsRegionCode(marketCode, localityId, countyId)
  const scopeKey = `${marketCode}:${regionCode}`

  const loading = useGenerationLoading(scopeKey, async (isCurrent) => {
    try {
      const data = await getBusinessPageSections(marketCode, regionCode)
      if (!isCurrent()) {
        return
      }
      ...
      const message = error instanceof Error ? error.message : t('businessPage.loadFailed')
  ...
  async function persistRows(nextRows: IEditableSectionRow[] = rows): Promise<void> {
    ...
    setSaving(true)
    try {
      const data = await putBusinessPageSections(marketCode, items, regionCode, toApiAdRows(adRows))
      setRows(toEditableRows(data.items))
      setAdRows(toEditableAdRows(data.ads))
      notifyEditorialPreviewStale(businessEditorScope(marketCode, localityId, countyId))
      pushToast(t('businessPage.saveSuccess'), 'success')
    } catch (error) {
      const message = error instanceof Error ? error.message : t('businessPage.saveFailed')
      pushToast(message, 'error')
    } finally {
      setSaving(false)
    }
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`businessPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        <p className="text-sm text-neutral-600">{t('businessPage.loading')}</p>
      ) : (
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('businessPage.loading')}</p>
          ) : null}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('businessPage.removeConfirm', { label: pendingRemoveLabel })}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {saving ? t('businessPage.saving') : t('businessPage.save')}
        </button>
      </div>
    </div>
  )
...
/**
 * Editable ordered list of business page sections (hero, bands, business).
 */
function BusinessRowsEditor({
  rows,
  onChange,
  onRemove,
}: {
  ...
  function addSection(sectionType: BusinessPageSectionType): void {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-neutral-600">{t('businessPage.listHint')}</p>
      <ul className="space-y-2">
        {rows.map((row, index) => (
          <SortableConfigRow
            ...
            dragHandleLabel={t('businessPage.dragHandle')}
      ...
      {preview && previewRow ? (
        <SortableDragPreview preview={preview}>
          <BusinessRowPreview row={previewRow} index={preview.index} />
        </SortableDragPreview>
      ) : null}
      <div className="flex flex-wrap gap-2">
        {SECTION_TYPES.map((sectionType) => {
          ...
          return (
            ...
            >
              {t(`businessPage.addType.${sectionType}`)}
            </button>
          )
        })}
      </div>
...
interface IBusinessRowFieldsProps {
...
/**
 * Inline editors and move/remove controls for one business section row.
 */
function BusinessRowFields(props: IBusinessRowFieldsProps): JSX.Element {
  ...
  return (
    <>
      ...
      <span className="rounded bg-neutral-100 px-2 py-1 text-xs font-semibold uppercase tracking-wide text-neutral-700">
        {t(`businessPage.types.${row.sectionType}`)}
      </span>
      <input
        value={row.label}
        onChange={(event) =>
        ...
        placeholder={t('businessPage.labelPlaceholder')}
        className={`${INPUT_CLASS} min-w-[12rem] flex-1`}
        aria-label={t('businessPage.labelPlaceholder')}
      />
...
/**
 * Compact floating preview content for a dragged business row.
 */
function BusinessRowPreview({
  row,
  index,
}: {
  row: IEditableSectionRow
  ...
  return (
    <>
      ...
      <span className="min-w-[12rem] flex-1 truncate text-sm font-medium text-neutral-900">
        {row.label || t('businessPage.labelPlaceholder')}
      </span>
    </>
  )
}
