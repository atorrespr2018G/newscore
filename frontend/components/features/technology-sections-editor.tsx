...
import {
  getTechnologyPageSections,
  putTechnologyPageSections,
  type ITechnologyPageSectionItem,
  type TechnologyPageSectionType,
...
const SECTION_TYPES: TechnologyPageSectionType[] = [
  'hero',
  'top_stories',
  'live',
  'archive',
...
const DEFAULT_LABEL_BY_TYPE: Record<TechnologyPageSectionType, string> = {
  hero: 'Technology',
...
const CANONICAL_SLUG_BY_TYPE: Partial<Record<TechnologyPageSectionType, string>> = {
  hero: 'hero',
...
const PREFERRED_SLUG_PREFIX_BY_TYPE: Partial<Record<TechnologyPageSectionType, string>> = {
  ribbon_ad: 'ad-ribbon',
}

interface IEditableSectionRow {
  key: string
  sectionType: TechnologyPageSectionType
  label: string
  slug: string
}
 ...
 *
 * @param items Saved technology section items.
 * @returns Editable row models with stable keys.
 */
function toEditableRows(items: ITechnologyPageSectionItem[]): IEditableSectionRow[] {
...
/**
 * Region code for technology section scope (country, state, county, or town).
 *
 ...
 */
function technologySectionsRegionCode(
  marketCode: string,
...
/**
 * Build the editor scope used to invalidate Placement after a technology save.
 *
 * @param marketCode Active market code.
 * @param localityId US state or PR town short code.
 * @param countyId Optional Florida county slug.
 * @returns Scope matching the technology board that was just written.
 */
function technologyEditorScope(
  marketCode: string,
  localityId: string | null,
  countyId: string | null,
): IEditorScope {
...
/**
 * Admin editor for the ordered technology page section list of one geo scope.
 *
 * @returns Technology sections administration UI.
 */
export function TechnologySectionsEditor(): JSX.Element {
  ...
  const regionCode = technologySectionsRegionCode(marketCode, localityId, countyId)
  const scopeKey = `${marketCode}:${regionCode}`

  const loading = useGenerationLoading(scopeKey, async (isCurrent) => {
    try {
      const data = await getTechnologyPageSections(marketCode, regionCode)
      if (!isCurrent()) {
        return
      }
      ...
      const message = error instanceof Error ? error.message : t('technologyPage.loadFailed')
  ...
  async function persistRows(nextRows: IEditableSectionRow[] = rows): Promise<void> {
    ...
    setSaving(true)
    try {
      const data = await putTechnologyPageSections(
        marketCode,
        items,
        regionCode,
        toApiAdRows(adRows),
      ...
      setRows(toEditableRows(data.items))
      setAdRows(toEditableAdRows(data.ads))
      notifyEditorialPreviewStale(technologyEditorScope(marketCode, localityId, countyId))
      pushToast(t('technologyPage.saveSuccess'), 'success')
    } catch (error) {
      const message = error instanceof Error ? error.message : t('technologyPage.saveFailed')
      pushToast(message, 'error')
    } finally {
      setSaving(false)
    }
  ...
  const pendingRemoveLabel = pendingRemoveRow
    ? pendingRemoveRow.label.trim() || t(`technologyPage.types.${pendingRemoveRow.sectionType}`)
    : ''
  ...
  return (
    <div className="mt-4 space-y-4">
      ...
      {loading && rows.length === 0 ? (
        <p className="text-sm text-neutral-600">{t('technologyPage.loading')}</p>
      ) : (
        <>
          {loading ? (
            <p className="mb-2 text-xs text-neutral-500">{t('technologyPage.loading')}</p>
          ) : null}
          <div className={loading ? 'pointer-events-none opacity-60' : undefined}>
            <TechnologyRowsEditor
              rows={rows}
              onChange={setRows}
      ...
      {pendingRemoveRow ? (
        <ConfirmDialog
          ...
          message={t('technologyPage.removeConfirm', { label: pendingRemoveLabel })}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {saving ? t('technologyPage.saving') : t('technologyPage.save')}
        </button>
      </div>
    </div>
  )
...
/**
 * Editable ordered list of technology page sections (hero, bands, archive, ads).
 */
function TechnologyRowsEditor({
  rows,
  onChange,
  onRemove,
}: {
  ...
  const hasArchive = rows.some((row) => row.sectionType === 'archive')
  const {
    dragIndex,
    overIndex,
    preview,
  ...
  function addSection(sectionType: TechnologyPageSectionType): void {
    if (sectionType === 'hero' && hasHero) {
      return
    }
    if (sectionType === 'archive' && hasArchive) {
      return
    }
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-neutral-600">{t('technologyPage.listHint')}</p>
      <ul className="space-y-2">
        {rows.map((row, index) => (
          <SortableConfigRow
            ...
            dragHandleLabel={t('technologyPage.dragHandle')}
      ...
      {preview && previewRow ? (
        <SortableDragPreview preview={preview}>
          <TechnologyRowPreview row={previewRow} index={preview.index} />
        </SortableDragPreview>
      ) : null}
      <div className="flex flex-wrap gap-2">
        {SECTION_TYPES.map((sectionType) => {
          const disabled =
            (sectionType === 'hero' && hasHero) || (sectionType === 'archive' && hasArchive)
          return (
            <button
              key={sectionType}
              type="button"
            ...
            >
              {t(`technologyPage.addType.${sectionType}`)}
            </button>
          )
        })}
      </div>
...
interface ITechnologyRowFieldsProps {
...
/**
 * Inline editors and move/remove controls for one technology section row.
 */
function TechnologyRowFields(props: ITechnologyRowFieldsProps): JSX.Element {
  ...
  return (
    <>
      ...
      <span className="rounded bg-neutral-100 px-2 py-1 text-xs font-semibold uppercase tracking-wide text-neutral-700">
        {t(`technologyPage.types.${row.sectionType}`)}
      </span>
      <input
        value={row.label}
        onChange={(event) =>
        ...
        placeholder={t('technologyPage.labelPlaceholder')}
        className={`${INPUT_CLASS} min-w-[12rem] flex-1`}
        aria-label={t('technologyPage.labelPlaceholder')}
      />
...
/**
 * Compact floating preview content for a dragged technology row.
 */
function TechnologyRowPreview({
  row,
  index,
}: {
  row: IEditableSectionRow
  ...
  return (
    <>
      ...
      <span className="min-w-[12rem] flex-1 truncate text-sm font-medium text-neutral-900">
        {row.label || t('technologyPage.labelPlaceholder')}
      </span>
    </>
  )
}
