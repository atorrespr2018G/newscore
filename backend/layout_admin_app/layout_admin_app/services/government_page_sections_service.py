"""Per-market / per-region government page section list CRUD and layout sync."""
...
from shared.core.government_page_sections_sync import (
    ...
    insert_legacy_government_ribbon_ads,
...
from shared.schemas.government_page_sections_schemas import (
    GovernmentPageSectionItemIn,
    GovernmentPageSectionItemOut,
...
def _resolve_item_slug(
    *,
    ...
    return slugify_government_label(label)


def _normalize_items(items: list[GovernmentPageSectionItemIn]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_TOPIC and slug in PRESERVED_GOVERNMENT_PAGE_KEYS:
        ...
        if section_type != SECTION_TYPE_TOPIC:
            ...
            if expected and slug == expected:
                ...
                and slug in PRESERVED_GOVERNMENT_PAGE_KEYS
                and slug != expected
            ):
...
def _sections_query(*, market_id: str, region_id: str | None) -> dict[str, Any]:
    """Build the unique-scope filter for a government sections document."""
...
async def _upsert_sections_doc(
    ...
    *,
    ...
    """Insert or replace the government sections document for one scope."""

    query = _sections_query(market_id=market_id, region_id=region_id)
    existing = await db[GOVERNMENT_PAGE_SECTIONS_COLLECTION].find_one(query, {"_id": 1})
    payload = {
        "items": items,
        "ads": ads,
        "updated_at": now,
...
def _to_out(
    *,
    ...
    return GovernmentPageSectionsOut(
        market_id=market_id,
...
async def get_for_market(
    ...
    *,
    ...
    """Return the government section list for a market and optional region."""
    ...
    ads = resolve_ads_list(doc.get("ads"), page_name=PAGE_NAME_GOVERNMENT)
    now = utc_now().isoformat()
    if not doc.get(RIBBON_ADS_MIGRATED_FIELD) and not has_ribbon_ad_section(items):
        items = insert_legacy_government_ribbon_ads(items)
...
async def replace_for_market(
    ...
    *,
    ...
    """Replace the government section list and sync the matching government layout."""
    ...
    if body.ads is None:
        ...
        ads = resolve_ads_list(
            ...
            page_name=PAGE_NAME_GOVERNMENT,
        )
    else:
    ...
    logger.info(
        "Updated government page sections for market %s region %s (%d items)",
        market_code,
        normalized_region,
        len(items),
    )
