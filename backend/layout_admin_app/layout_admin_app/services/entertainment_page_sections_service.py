"""Per-market / per-region entertainment page section list CRUD and layout sync."""
...
from shared.core.entertainment_page_sections_sync import (
    ...
    insert_legacy_entertainment_ribbon_ads,
...
from shared.schemas.entertainment_page_sections_schemas import (
    EntertainmentPageSectionItemIn,
    EntertainmentPageSectionItemOut,
...
def _resolve_item_slug(
    *,
    ...
    return slugify_entertainment_label(label)


def _normalize_items(items: list[EntertainmentPageSectionItemIn]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_ENTERTAINMENT and slug in PRESERVED_ENTERTAINMENT_PAGE_KEYS:
            raise ValidationError(f"Entertainment slug '{slug}' is reserved")
        ...
        if section_type != SECTION_TYPE_ENTERTAINMENT:
            ...
            if expected and slug == expected:
                ...
                and slug in PRESERVED_ENTERTAINMENT_PAGE_KEYS
                and slug != expected
            ):
...
def _sections_query(*, market_id: str, region_id: str | None) -> dict[str, Any]:
    """Build the unique-scope filter for a entertainment sections document."""
...
async def _upsert_sections_doc(
    ...
    *,
    ...
    """Insert or replace the entertainment sections document for one scope."""

    query = _sections_query(market_id=market_id, region_id=region_id)
    existing = await db[ENTERTAINMENT_PAGE_SECTIONS_COLLECTION].find_one(query, {"_id": 1})
    payload = {
        "items": items,
        "ads": ads,
        "updated_at": now,
...
def _to_out(
    *,
    ...
    return EntertainmentPageSectionsOut(
        market_id=market_id,
...
async def _fallback_entertainment_items(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str | None,
) -> tuple[list[dict[str, str]], list[dict[str, Any]]]:
    """Resolve items when the requested entertainment board is missing.
    ...
    if region_id is not None:
        source = await db[ENTERTAINMENT_PAGE_SECTIONS_COLLECTION].find_one(
            _sections_query(market_id=market_id, region_id=None),
        )
        if source is not None:
            items = expand_legacy_section_items(list(source.get("items") or []))
            ads = resolve_ads_list(source.get("ads"), page_name=PAGE_NAME_ENTERTAINMENT)
            return items, ads
    items = default_entertainment_page_section_items(list(DEFAULT_ENTERTAINMENT_TOPIC_LABELS))
    return items, default_ads_for_page(PAGE_NAME_ENTERTAINMENT)


async def _persist_entertainment_board(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str | None,
    ...
    """Write an entertainment board and sync its layout slots.
...
async def get_for_market(
    ...
    *,
    ...
    """Return the entertainment section list for a market and optional region."""
    ...
    doc = await db[ENTERTAINMENT_PAGE_SECTIONS_COLLECTION].find_one(
        _sections_query(market_id=market_id, region_id=region_id),
    )
    if doc is None:
        items, ads = await _fallback_entertainment_items(
            db,
            market_id=market_id,
            region_id=region_id,
        )
        now = await _persist_entertainment_board(
            db,
            market_id=market_id,
            region_id=region_id,
            items=items,
    ...
    ads = resolve_ads_list(doc.get("ads"), page_name=PAGE_NAME_ENTERTAINMENT)
    now = utc_now().isoformat()
    if not doc.get(RIBBON_ADS_MIGRATED_FIELD) and not has_ribbon_ad_section(items):
        items = insert_legacy_entertainment_ribbon_ads(items)
        await _upsert_sections_doc(
            db,
            market_id=market_id,
            region_id=region_id,
...
async def replace_for_market(
    ...
    *,
    ...
    """Replace the entertainment section list and sync the matching entertainment layout."""
    ...
    if body.ads is None:
        ...
        ads = resolve_ads_list(
            ...
            page_name=PAGE_NAME_ENTERTAINMENT,
        )
    else:
    ...
    logger.info(
        "Updated entertainment page sections for market %s region %s (%d items)",
        market_code,
        normalized_region,
        len(items),
    )
