"""Per-market / per-region Politics page section list CRUD and layout sync."""
...
from shared.core.politics_page_sections_sync import (
    ...
    insert_legacy_politics_ribbon_ads,
    slugify_section_label,
    sync_politics_layout_slots,
)

from shared.models.common import utc_now
from shared.read.collections import POLITICS_PAGE_SECTIONS_COLLECTION
...
from shared.schemas.politics_page_sections_schemas import (
    PoliticsPageSectionItemIn,
    PoliticsPageSectionItemOut,
...
def _normalize_items(items: list[PoliticsPageSectionItemIn]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_CATEGORY and slug in PRESERVED_POLITICS_PAGE_KEYS:
        ...
        if section_type != SECTION_TYPE_CATEGORY:
            ...
            if expected and slug == expected:
                ...
                and slug in PRESERVED_POLITICS_PAGE_KEYS
                and slug != expected
            ):
...
def _sections_query(*, market_id: str, region_id: str | None) -> dict[str, Any]:
    """Build the unique-scope filter for a Politics sections document."""
...
async def _upsert_sections_doc(
    ...
    *,
    ...
    """Insert or replace the Politics sections document for one scope."""

    query = _sections_query(market_id=market_id, region_id=region_id)
    existing = await db[POLITICS_PAGE_SECTIONS_COLLECTION].find_one(query, {"_id": 1})
    payload = {
        "items": items,
        "ads": ads,
        "updated_at": now,
    ...
    if existing is not None:
        await db[POLITICS_PAGE_SECTIONS_COLLECTION].update_one(
            {"_id": existing["_id"]},
            {"$set": payload},
        )
        return

    await db[POLITICS_PAGE_SECTIONS_COLLECTION].insert_one(
        {
            "_id": str(uuid4()),
            "market_id": market_id,
            **payload,
...
def _to_out(
    *,
    ...
    return PoliticsPageSectionsOut(
        market_id=market_id,
...
async def get_for_market(
    ...
    *,
    ...
    """Return the Politics-page section list for a market and optional region."""
    ...
    doc = await db[POLITICS_PAGE_SECTIONS_COLLECTION].find_one(
        _sections_query(market_id=market_id, region_id=region_id),
    )
    if doc is None:
        items = [dict(row) for row in DEFAULT_POLITICS_SECTION_ITEMS]
        ads = default_ads_for_page(PAGE_NAME_POLITICS)
        return _to_out(
            market_id=market_id,
    ...
    items = expand_politics_section_items(list(doc.get("items") or []))
    ads = resolve_ads_list(doc.get("ads"), page_name=PAGE_NAME_POLITICS)
    now = utc_now().isoformat()
    if not doc.get(RIBBON_ADS_MIGRATED_FIELD) and not has_ribbon_ad_section(items):
        items = insert_legacy_politics_ribbon_ads(items)
        ...
        await sync_politics_layout_slots(
            db,
            market_id=market_id,
            items=items,
            region_id=region_id,
...
async def replace_for_market(
    ...
    *,
    ...
    """Replace the Politics-page section list and sync the matching Politics layout."""
    ...
    if body.ads is None:
        existing = await db[POLITICS_PAGE_SECTIONS_COLLECTION].find_one(
            _sections_query(market_id=market_id, region_id=region_id),
        )
        ads = resolve_ads_list(
            None if existing is None else existing.get("ads"),
            page_name=PAGE_NAME_POLITICS,
        )
    else:
    ...
    logger.info(
        "Updated Politics page sections for market %s region %s (%d items)",
        market_code,
        normalized_region,
        len(items),
    )
