"""Per-market / per-region health page section list CRUD and layout sync."""
...
from shared.core.health_page_sections_sync import (
    ...
    expand_legacy_health_section_items,
    has_ribbon_ad_section,
    insert_legacy_health_ribbon_ads,
...
from shared.schemas.health_page_sections_schemas import (
    HealthPageSectionItemIn,
    HealthPageSectionItemOut,
...
def _resolve_item_slug(
    *,
    ...
    return slugify_health_label(label)


def _normalize_items(items: list[HealthPageSectionItemIn]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_HEALTH and slug in PRESERVED_HEALTH_PAGE_KEYS:
            raise ValidationError(f"Health slug '{slug}' is reserved")
        ...
        if section_type != SECTION_TYPE_HEALTH:
            ...
            if expected and slug == expected:
                ...
                and slug in PRESERVED_HEALTH_PAGE_KEYS
                and slug != expected
            ):
...
def _sections_query(*, market_id: str, region_id: str | None) -> dict[str, Any]:
    """Build the unique-scope filter for a health sections document."""
...
async def _upsert_sections_doc(
    ...
    *,
    ...
    """Insert or replace the health sections document for one scope."""

    query = _sections_query(market_id=market_id, region_id=region_id)
    existing = await db[HEALTH_PAGE_SECTIONS_COLLECTION].find_one(query, {"_id": 1})
    payload = {
        "items": items,
        "ads": ads,
        "updated_at": now,
...
async def _fallback_health_items(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str | None,
) -> tuple[list[dict[str, str]], list[dict[str, Any]]]:
    """Resolve items when the requested health board is missing.
    ...
    if region_id is not None:
        source = await db[HEALTH_PAGE_SECTIONS_COLLECTION].find_one(
            _sections_query(market_id=market_id, region_id=None),
        )
        if source is not None:
            items = expand_legacy_health_section_items(list(source.get("items") or []))
            ads = resolve_ads_list(source.get("ads"), page_name=PAGE_NAME_HEALTH)
            return items, ads
    items = default_health_page_section_items(list(DEFAULT_HEALTH_TOPIC_LABELS))
    return items, default_ads_for_page(PAGE_NAME_HEALTH)


async def _persist_health_board(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str | None,
    ...
    """Write an health board and sync its layout slots.
...
async def get_for_market(
    ...
    *,
    ...
    """Return the health section list for a market and optional region."""
    ...
    doc = await db[HEALTH_PAGE_SECTIONS_COLLECTION].find_one(
        _sections_query(market_id=market_id, region_id=region_id),
    )
    if doc is None:
        items, ads = await _fallback_health_items(
            db,
            market_id=market_id,
            region_id=region_id,
        )
        now = await _persist_health_board(
            db,
            market_id=market_id,
            region_id=region_id,
            items=items,
    ...
    items = expand_legacy_health_section_items(list(doc.get("items") or []))
    ads = resolve_ads_list(doc.get("ads"), page_name=PAGE_NAME_HEALTH)
    now = utc_now().isoformat()
    if not doc.get(RIBBON_ADS_MIGRATED_FIELD) and not has_ribbon_ad_section(items):
        items = insert_legacy_health_ribbon_ads(items)
        await _upsert_sections_doc(
            db,
            market_id=market_id,
            region_id=region_id,
...
async def replace_for_market(
    ...
    *,
    ...
    """Replace the health section list and sync the matching health layout."""
    ...
    logger.info(
        "Updated health page sections for market %s region %s (%d items)",
        market_code,
        normalized_region,
        len(items),
    )
