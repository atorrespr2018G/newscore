"""Per-market / per-region World page section list CRUD and layout sync."""
...
from shared.schemas.world_page_sections_schemas import (
    WorldPageSectionItemIn,
    WorldPageSectionItemOut,
...
def _normalize_items(items: list[WorldPageSectionItemIn]) -> list[dict[str, str]]:
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_CATEGORY and slug in PRESERVED_WORLD_PAGE_KEYS:
        ...
        if section_type != SECTION_TYPE_CATEGORY:
            ...
            if expected and slug == expected:
                ...
                and slug in PRESERVED_WORLD_PAGE_KEYS
                and slug != expected
            ):
...
def _sections_query(*, market_id: str, region_id: str | None) -> dict[str, Any]:
    """Build the unique-scope filter for a World sections document."""
...
async def _upsert_sections_doc(
    ...
    *,
    ...
    existing = await db[WORLD_PAGE_SECTIONS_COLLECTION].find_one(query, {"_id": 1})
    payload = {
        "items": items,
        "ads": ads,
        "updated_at": now,
    ...
    await db[WORLD_PAGE_SECTIONS_COLLECTION].insert_one(
        {
            "_id": str(uuid4()),
            "market_id": market_id,
            **payload,
...
async def get_for_market(
    ...
    *,
    ...
    """Return the World-page section list for a market and optional region."""
    ...
    doc = await db[WORLD_PAGE_SECTIONS_COLLECTION].find_one(
        _sections_query(market_id=market_id, region_id=region_id),
    )
    if doc is None:
        items = [dict(row) for row in DEFAULT_WORLD_SECTION_ITEMS]
        ads = default_ads_for_page(PAGE_NAME_WORLD)
        return _to_out(
            market_id=market_id,
...
async def replace_for_market(
    ...
    *,
    ...
    """Replace the World-page section list and sync the matching World layout."""
    ...
    if body.ads is None:
        existing = await db[WORLD_PAGE_SECTIONS_COLLECTION].find_one(
            _sections_query(market_id=market_id, region_id=region_id),
        )
        ads = resolve_ads_list(
            None if existing is None else existing.get("ads"),
            page_name=PAGE_NAME_WORLD,
        )
    else:
    ...
    logger.info(
        "Updated world page sections for market %s region %s (%d items)",
        market_code,
        normalized_region,
        len(items),
    )
