"""Remove the Elections row from homepage layouts only.

Politics page Elections (``politics-spotlight``) is left unchanged.
"""
...
def _strip_election_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Drop the homepage Elections row and a ribbon immediately before it."""

    migrated: list[dict[str, Any]] = []
    for item in items:
        slug = str(item.get("slug") or "")
        if slug == ELECTION_POSITION_KEY:
...
async def _migrate_layout(db: Any, layout_id: str) -> bool:
    """Delete homepage Elections slots and compact remaining order indexes."""
    ...
    for slot in slots:
        ...
        if position_key == ELECTION_POSITION_KEY:
            if kept and _is_ribbon_slot(kept[-1]):
...
async def _migrate_sections_docs(db: Any) -> int:
    """Strip Elections rows from homepage_page_sections documents."""

    updated = 0
    async for doc in db[HOMEPAGE_PAGE_SECTIONS_COLLECTION].find({}):
        items = list(doc.get("items") or [])
        migrated = _strip_election_items(items)
        if migrated == items:
            continue
...
async def run() -> dict[str, int | bool]:
    """Remove Elections from homepage boards across markets and regions."""
