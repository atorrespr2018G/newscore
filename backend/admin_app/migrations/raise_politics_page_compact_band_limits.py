"""Raise Politics page topic-row limits from 6 to 12.

Aligns Policy, Courts & Law, State Politics, and Opinion with the homepage
Elections compact carousel so left/right arrows can paginate.
"""
...
COMPACT_BAND_POSITION_KEYS = (
    "politics-latest",
    "politics-courts",
    "politics-state",
    "politics-opinion",
)
COMPACT_BAND_EXTENDED_LIMIT = 12
...
async def _politics_layout_ids(db: Any) -> list[str]:
    """Return ids for active politics page layouts (market and region boards)."""

    cursor = db[LAYOUTS_COLLECTION].find(
        {"page_name": POLITICS_PAGE_NAME, "is_active": True},
        {"_id": 1},
    )
...
async def run() -> dict[str, Any]:
    """Update query_rule.limit for Politics page compact carousel slots."""

    client = AsyncIOMotorClient(_mongo_uri())
    try:
        db = client[_mongo_db_name()]
        layout_ids = await _politics_layout_ids(db)
        if not layout_ids:
        ...
        result = await db[SLOTS_COLLECTION].update_many(
            {
                ...
                "position_key": {"$in": list(COMPACT_BAND_POSITION_KEYS)},
            },
            {
                "$set": {
                    "query_rule.limit": COMPACT_BAND_EXTENDED_LIMIT,
