"""Shared helpers to sync Politics page section lists into layouts/slots."""
...
SPOTLIGHT_POSITION_KEY = "politics-spotlight"
...
POLITICS_PAGE_SECTION_TYPES = frozenset(
    {
        SECTION_TYPE_HERO,
...
PRESERVED_POLITICS_PAGE_KEYS = frozenset(
    {
        HERO_POSITION_KEY,
...
POLITICS_COMPACT_BAND_PAIR_SECONDS = frozenset({"politics-courts", "politics-opinion"})
...
def insert_legacy_politics_ribbon_ads(items: list[dict[str, str]]) -> list[dict[str, str]]:
    """Insert ribbon rows matching the former Politics-page heuristic placements.
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_CATEGORY:
            seen_category = True
            if slug in POLITICS_COMPACT_BAND_PAIR_SECONDS:
                ribbon_slug = _next_ribbon_slug(used_slugs)
                used_slugs.add(ribbon_slug)
                result.append(_ribbon_ad_item(ribbon_slug))
...
# Defaults match POLITICS_PAGE_SLOT_SPECS from seed_dev, including heuristic ad ribbons.
DEFAULT_POLITICS_SECTION_ITEMS: list[dict[str, str]] = insert_legacy_politics_ribbon_ads(
    [
        {"section_type": SECTION_TYPE_HERO, "slug": HERO_POSITION_KEY, "label": "Politics"},
        {
        ...
        {"section_type": SECTION_TYPE_CATEGORY, "slug": "politics-latest", "label": "Policy"},
        {"section_type": SECTION_TYPE_CATEGORY, "slug": "politics-courts", "label": "Courts & Law"},
        {
            "section_type": SECTION_TYPE_CATEGORY,
            "slug": "politics-state",
            "label": "State Politics",
        },
        {"section_type": SECTION_TYPE_CATEGORY, "slug": "politics-opinion", "label": "Opinion"},
    ],
)

# Topic archives use dedicated category slugs; compact rows keep position keys.
REGION_CATEGORY_SLUG_BY_POSITION = {
    MORE_TOP_STORIES_POSITION_KEY: "congress",
    SPOTLIGHT_POSITION_KEY: "elections",
    RAIL_POSITION_KEY: "white-house",
}

POLITICS_REGION_SECTION_TYPES = frozenset(
    {
        SECTION_TYPE_MORE_TOP_STORIES,
        SECTION_TYPE_SPOTLIGHT,
        SECTION_TYPE_RAIL,
...
PARENT_POLITICS_CATEGORY_SLUG = "politics"
...
def region_category_slug_for_item(item: dict[str, str]) -> str | None:
    """Return the article-pool category slug for a Politics topic row.

    Args:
        item: Typed Politics section row with ``section_type`` and ``slug``.

    Returns:
        Category slug such as ``europe``, or None when the row is not a region.
    """

    if item["section_type"] not in POLITICS_REGION_SECTION_TYPES:
        return None
    slug = item["slug"]
    return REGION_CATEGORY_SLUG_BY_POSITION.get(slug) or slug


def expand_politics_section_items(items: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Normalize stored items; empty lists become the seed default stack.

    Args:
        items: Raw politics_page_sections items.
    ...
    if not items:
        return [dict(row) for row in DEFAULT_POLITICS_SECTION_ITEMS]

    resolved: list[dict[str, str]] = []
    for item in items:
        section_type = str(item.get("section_type") or SECTION_TYPE_CATEGORY).strip().lower()
        if section_type not in POLITICS_PAGE_SECTION_TYPES:
            section_type = SECTION_TYPE_CATEGORY
    ...
    return resolved or [dict(row) for row in DEFAULT_POLITICS_SECTION_ITEMS]


def default_POLITICS_PAGE_section_items() -> list[dict[str, str]]:
    """Return a copy of the seeded Politics-page section list.

    Returns:
        Default typed section items matching POLITICS_PAGE_SLOT_SPECS.
    """
...
async def _ensure_category(db: AsyncIOMotorDatabase, *, slug: str, label: str) -> str:
    """Ensure a Politics topic category exists under parent Politics."""

    parent = await db[CATEGORIES_COLLECTION].find_one({"slug": PARENT_POLITICS_CATEGORY_SLUG})
    parent_id = str(parent["_id"]) if parent and slug != PARENT_POLITICS_CATEGORY_SLUG else None
    ...
    logger.info("Created politics section category %s", slug)
    return category_id


async def _region_category_id(db: AsyncIOMotorDatabase, item: dict[str, str]) -> str | None:
    """Ensure and return the category id for a Politics topic section row.

    Args:
        db: Mongo database.
        item: Typed Politics section row.
...
async def _ensure_politics_layout(db: AsyncIOMotorDatabase, *, market_id: str) -> dict[str, Any]:
    """Ensure an active market-level Politics layout exists."""

    layout = await db[LAYOUTS_COLLECTION].find_one(
        {
            "page_name": POLITICS_PAGE_NAME,
            "market_id": market_id,
    ...
    logger.info("Created Politics layout for market %s", market_id)
    return layout


async def _ensure_region_politics_layout(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
) -> dict[str, Any]:
    """Ensure a region-owned Politics layout exists, cloning from market when needed.

    Args:
        db: Mongo database.
        market_id: Market document id.
        region_id: Region document id that must own the Politics board.

    Returns:
        Layout document for the region Politics page.

    Raises:
        ValidationError: When no Politics layout can be ensured.
    """

    from shared.core.layout_ensure import ensure_exact_page_layout

    await _ensure_politics_layout(db, market_id=market_id)
    ...
    if layout_id is None:
        raise ValidationError(f"Unable to ensure Politics layout for region {region_id}")

    layout = await db[LAYOUTS_COLLECTION].find_one({"_id": layout_id})
    if layout is None:
        raise ValidationError(f"Politics layout missing after ensure: {layout_id}")
    return layout
...
async def _upsert_layout_slot(
    ...
    *,
    ...
    """Create or update one Politics layout slot without wiping existing pins.
...
async def _delete_obsolete_politics_slots(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    keep_slot_ids: set[str],
) -> None:
    """Remove Politics slots that are no longer in the active section list."""
...
async def _apply_politics_slots_to_layout(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    ...
    """Upsert Politics slots onto one layout and delete obsolete rows.
    ...
    await _delete_obsolete_politics_slots(db, layout_id=layout_id, keep_slot_ids=set(slot_ids))
    await db[LAYOUTS_COLLECTION].update_one(
        {"_id": layout_id},
...
async def _slot_spec_for_section(
    ...
    *,
    ...
    """Build one layout slot spec from a typed Politics-page section item."""
...
async def sync_politics_layout_slots(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    ...
    """Rebuild Politics slots from an ordered typed section list.

    Upserts slots in list order and deletes obsolete slots. Existing pins are
    preserved per ``position_key``.

    When ``region_id`` is set, updates only that region's Politics layout.
    Otherwise updates the market-level Politics layout only.
    ...
    resolved_items = expand_politics_section_items(items)
    if region_id:
        layout = await _ensure_region_politics_layout(
            db,
            market_id=market_id,
            region_id=region_id,
        )
    else:
        layout = await _ensure_politics_layout(db, market_id=market_id)
    ...
    await _apply_politics_slots_to_layout(
        db,
        layout_id=layout_id,
