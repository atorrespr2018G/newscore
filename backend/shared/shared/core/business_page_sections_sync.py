"""Shared helpers to sync business page section lists into layouts/slots."""
...
# Shared live carousel key across section pages (avoids colliding with parent slug).
LIVE_POSITION_KEY = "health"
PARENT_BUSINESS_CATEGORY_SLUG = "business"
PRESERVED_BUSINESS_PAGE_KEYS = frozenset(
    {
        HERO_POSITION_KEY,
        US_FEATURED_POSITION_KEY,
        LIVE_POSITION_KEY,
        PARENT_BUSINESS_CATEGORY_SLUG,
    },
)
...
def insert_legacy_business_ribbon_ads(items: list[dict[str, str]]) -> list[dict[str, str]]:
    """Insert ribbon rows matching the former business-page heuristic placements.
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_LIVE:
            ...
            section_type == SECTION_TYPE_BUSINESS
            and compact_index > 0
            and compact_index % BUSINESS_SECTION_PAIR_SIZE == 0
        ):
            needs_before = True
        elif (
        ...
        if section_type == SECTION_TYPE_HERO:
            ...
            used_slugs.add(ribbon_slug)
            result.append(_ribbon_ad_item(ribbon_slug))
        if section_type == SECTION_TYPE_BUSINESS:
            compact_index += 1
...
DEFAULT_FIXED_SECTION_ITEMS: list[dict[str, str]] = insert_legacy_business_ribbon_ads(
    [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS],
)
# Configuration topic rows matching Economía beat categories (no World).
DEFAULT_BUSINESS_TOPIC_LABELS: tuple[str, ...] = (
    "Economy",
    "Companies",
    "Banking",
    "Autos",
...
BUSINESS_SECTION_ARTICLE_LIMIT = 12
BUSINESS_HERO_ARTICLE_LIMIT = 12
BUSINESS_TOP_STORIES_ARTICLE_LIMIT = 12
BUSINESS_LIVE_ARTICLE_LIMIT = 20
LIVE_CATEGORY_SLUG = "business"
_SLUG_SAFE_RE = re.compile(r"[^a-z0-9]+")


def slugify_business_label(label: str) -> str:
    """Derive a URL-safe business slug from a display label.

    Args:
        label: Human-readable business name.
    ...
    if not normalized:
        raise ValidationError("Business label must contain letters or numbers")
    return normalized


def expand_legacy_business_section_items(items: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Normalize stored items and prepend fixed sections for legacy business-only lists.

    Args:
        items: Raw business_page_sections items (may omit ``section_type``).
    ...
    if has_typed:
        ...
        for item in items:
            section_type = str(item.get("section_type") or SECTION_TYPE_BUSINESS).strip().lower()
            if section_type not in BUSINESS_PAGE_SECTION_TYPES:
                section_type = SECTION_TYPE_BUSINESS
            resolved.append(
                {
    ...
    return insert_legacy_business_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS] + business,
    )


def business_rows_from_labels(labels: list[str]) -> list[dict[str, str]]:
    """Build typed business rows from display labels.

    Args:
        labels: Ordered business display names.

    Returns:
        Business-typed section items with derived slugs.
    """

    return [
        {
            "section_type": SECTION_TYPE_BUSINESS,
            "slug": slugify_business_label(label),
            "label": label,
        }
        for label in labels
    ]
...
def default_business_page_section_items(labels: list[str]) -> list[dict[str, str]]:
    """Full default business page list: fixed bands plus business rows.

    Args:
        labels: Ordered business display names.

    Returns:
        Fixed sections followed by business rows, with legacy ad ribbons inserted.
    """

    return insert_legacy_business_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS]
        + business_rows_from_labels(labels),
    )


async def _ensure_business_category(db: AsyncIOMotorDatabase, *, slug: str, label: str) -> str:
    """Ensure a global category exists for a business slug under parent Business."""

    parent = await db[CATEGORIES_COLLECTION].find_one({"slug": PARENT_BUSINESS_CATEGORY_SLUG})
    parent_id = str(parent["_id"]) if parent else None
    if slug == PARENT_BUSINESS_CATEGORY_SLUG and parent is not None:
    ...
    if existing is not None:
        await db[CATEGORIES_COLLECTION].update_one(
            {"_id": existing["_id"]},
            {
                "$set": {
                    "name": label,
                    "description": f"{label} business news.",
                    "parent_id": parent_id,
                },
            },
        )
    ...
    logger.info("Created business category %s", slug)
    return category_id


async def _ensure_business_layout(db: AsyncIOMotorDatabase, *, market_id: str) -> dict[str, Any]:
    """Ensure an active market-level business layout exists (not a region clone)."""
    ...
    logger.info("Created business layout for market %s", market_id)
    return layout


async def _ensure_region_business_layout(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
) -> dict[str, Any]:
    """Ensure a region-owned business layout exists, cloning from market when needed.

    Args:
        db: Mongo database.
        market_id: Market document id.
        region_id: Region document id that must own the business board.

    Returns:
        Layout document for the region business page.

    Raises:
        ValidationError: When no market business layout exists to clone from.
    """
    ...
    await _ensure_business_layout(db, market_id=market_id)
    ...
    if layout_id is None:
        raise ValidationError(f"Unable to ensure business layout for region {region_id}")

    layout = await db[LAYOUTS_COLLECTION].find_one({"_id": layout_id})
    if layout is None:
        raise ValidationError(f"Business layout missing after ensure: {layout_id}")
    return layout


def _business_slot_query_rule(
    *,
    limit: int,
    existing: dict[str, Any] | None,
    category_id: str | None,
) -> dict[str, Any]:
    """Build query_rule so business slots auto-fill from their category.
...
async def _upsert_layout_slot(
    ...
    *,
    ...
    """Create or update one business layout slot without wiping existing pins."""

    existing = await db[SLOTS_COLLECTION].find_one(
        {"layout_id": layout_id, "position_key": position_key},
    )
    fields = {
        "query_rule": _business_slot_query_rule(
            limit=limit,
            existing=existing,
...
async def _delete_obsolete_business_slots(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    keep_slot_ids: set[str],
) -> None:
    """Remove business-page slots that are no longer in the active list."""
...
async def _apply_business_slots_to_layout(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    ...
    """Upsert business slots onto one layout and delete obsolete rows.
    ...
    await _delete_obsolete_business_slots(db, layout_id=layout_id, keep_slot_ids=set(slot_ids))
    await db[LAYOUTS_COLLECTION].update_one(
        {"_id": layout_id},
...
async def _slot_spec_for_section(
    ...
    *,
    ...
    """Build one layout slot spec from a typed business page section item."""
    ...
    if section_type == SECTION_TYPE_HERO:
        return {
            ...
            "limit": BUSINESS_HERO_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_TOP_STORIES:
        return {
            ...
            "category_id": business_category_id,
            "limit": BUSINESS_TOP_STORIES_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_LIVE:
        return {
            "position_key": slug,
            ...
            "limit": BUSINESS_LIVE_ARTICLE_LIMIT,
        }
    ...
    category_id = await _ensure_business_category(db, slug=slug, label=label)
    return {
        "position_key": slug,
        "order_index": order_index,
        "display_name": label,
        ...
        "category_id": category_id,
        "limit": BUSINESS_SECTION_ARTICLE_LIMIT,
    }


async def sync_business_layout_slots(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    ...
    """Rebuild business page slots from an ordered typed section list.

    Expands legacy business-only lists, then upserts slots in list order and deletes
    obsolete slots. Existing pins are preserved per ``position_key``.

    When ``region_id`` is set, updates only that region's business layout. Otherwise
    updates the market-level business layout only (does not mirror onto region boards).

    Args:
        db: Mongo database.
        market_id: Market document id.
        items: Ordered section rows (``section_type`` optional for legacy docs).
        region_id: Optional region document id for a state, county, or town business board.
    """

    resolved_items = expand_legacy_business_section_items(items)
    business_category_id = await _category_id_by_slug(db, PARENT_BUSINESS_CATEGORY_SLUG)
    live_category_id = await _category_id_by_slug(db, LIVE_CATEGORY_SLUG)
    if region_id:
        layout = await _ensure_region_business_layout(
            db,
            market_id=market_id,
            region_id=region_id,
        )
    else:
        layout = await _ensure_business_layout(db, market_id=market_id)
    ...
    await _apply_business_slots_to_layout(
        db,
        layout_id=layout_id,
...
async def _resolve_region_business_items(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
    ...
    """Load or create business section items for one region.
...
async def ensure_region_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    market_code: str,
    ...
    """Upsert business section lists and sync layouts for the given region codes.
    ...
    already auto-fill are not rewritten to pin-only business slots.

    Args:
        db: Mongo database.
        market_code: Market short code that owns the regions (e.g. ``us``, ``pr``).
        region_codes: Ordered region codes to ensure.
        labels: Ordered business display labels (typically PR business labels).
    ...
    if market is None:
        raise ValidationError(
            f"{market_code.upper()} market not found; cannot seed business sections",
        )

    market_id = str(market["_id"])
    default_items = default_business_page_section_items(labels)
    now = utc_now().isoformat()
    ...
    for region_code in region_codes:
        ...
        if region is None:
            logger.warning("Skipping business sections; region missing: %s", region_code)
            continue
        region_id = str(region["_id"])
        items, created, should_sync = await _resolve_region_business_items(
            db,
            market_id=market_id,
            region_id=region_id,
        ...
        if created:
            created_count += 1
        if should_sync:
            await sync_business_layout_slots(
                db,
                market_id=market_id,
                items=items,
                region_id=region_id,
    ...
    logger.info(
        "Ensured %s business sections for %d regions (%d created, %d business)",
        market_code,
        len(ensured_codes),
        created_count,
        len(labels),
...
async def ensure_us_state_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert PR-shaped business section lists and sync layouts for every US state.
    ...
    return await ensure_region_business_sections(
        db,
        market_code="us",
...
async def ensure_florida_county_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert business section lists and sync layouts for every Florida county.
...
async def ensure_pr_town_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert business section lists and sync layouts for every Puerto Rico town.
...
async def ensure_country_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert business lists for the US and Puerto Rico country boards.

    Configuration sends region ``us`` / ``pr`` when no state or town is selected.

    Args:
        db: Mongo database.
        labels: Ordered business display labels.

    Returns:
        Summary keyed by country code.
    """

    us_board = await ensure_region_business_sections(
        db,
        market_code="us",
        region_codes=("us",),
        labels=labels,
    )
    pr_board = await ensure_region_business_sections(
        db,
        market_code="pr",
        region_codes=("pr",),
        labels=labels,
...
async def ensure_geo_business_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Ensure business lists for countries, US states, FL counties, and PR towns.
    ...
    countries = await ensure_country_business_sections(db, labels=labels)
    states = await ensure_us_state_business_sections(db, labels=labels)
    counties = await ensure_florida_county_business_sections(db, labels=labels)
    towns = await ensure_pr_town_business_sections(db, labels=labels)
    return {
        "countries": countries,
        "us_states": states,
