"""Shared helpers to sync government page section lists into layouts/slots."""
...
GOVERNMENT_PAGE_SECTION_TYPES = frozenset(
    {
        SECTION_TYPE_HERO,
...
def insert_legacy_government_ribbon_ads(items: list[dict[str, str]]) -> list[dict[str, str]]:
    """Insert ribbon rows matching the former government-page heuristic placements.
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_LIVE:
            ...
            and compact_index > 0
            and compact_index % GOVERNMENT_SECTION_PAIR_SIZE == 0
        ):
            needs_before = True
        elif (
...
DEFAULT_FIXED_SECTION_ITEMS: list[dict[str, str]] = insert_legacy_government_ribbon_ads(
    [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS],
)

GOVERNMENT_SECTION_ARTICLE_LIMIT = 12
GOVERNMENT_HERO_ARTICLE_LIMIT = 12
GOVERNMENT_TOP_STORIES_ARTICLE_LIMIT = 12
GOVERNMENT_LIVE_ARTICLE_LIMIT = 20
GOVERNMENT_WORLD_ARTICLE_LIMIT = 12
PARENT_GOVERNMENT_CATEGORY_SLUG = "government"
LIVE_CATEGORY_SLUG = "health"
# Distinct from homepage/world-page `world` so Government World is its own article pool.
GOVERNMENT_WORLD_CATEGORY_SLUG = "government-page-world"
GOVERNMENT_WORLD_CATEGORY_NAME = "World"
GOVERNMENT_WORLD_CATEGORY_DESCRIPTION = "World news curated for the Government page only."
_SLUG_SAFE_RE = re.compile(r"[^a-z0-9]+")


def slugify_government_label(label: str) -> str:
...
def expand_legacy_section_items(items: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Normalize stored items and prepend fixed sections for legacy topic-only lists.

    Args:
        items: Raw government_page_sections items (may omit ``section_type``).
    ...
    if has_typed:
        ...
        for item in items:
            ...
            if section_type not in GOVERNMENT_PAGE_SECTION_TYPES:
                section_type = SECTION_TYPE_TOPIC
            resolved.append(
                {
    ...
    return insert_legacy_government_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS] + government,
    )
...
def topic_rows_from_labels(labels: list[str]) -> list[dict[str, str]]:
    ...
    return [
        {
            "section_type": SECTION_TYPE_TOPIC,
            "slug": slugify_government_label(label),
            "label": label,
        }
        for label in labels
    ]
...
def default_government_page_section_items(labels: list[str]) -> list[dict[str, str]]:
    """Full default government page list: fixed bands plus topic rows.
    ...
    return insert_legacy_government_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS]
        + topic_rows_from_labels(labels),
    )


async def _ensure_government_topic_category(db: AsyncIOMotorDatabase, *, slug: str, label: str) -> str:
    """Ensure a global category exists for a topic slug under parent Government."""

    parent = await db[CATEGORIES_COLLECTION].find_one({"slug": PARENT_GOVERNMENT_CATEGORY_SLUG})
    ...
    if existing is not None:
        await db[CATEGORIES_COLLECTION].update_one(
            {"_id": existing["_id"]},
            {
                "$set": {
                    "name": label,
                    "description": f"{label} government news.",
                    "parent_id": parent_id,
                },
            },
        )
    ...
    logger.info("Created topic category %s", slug)
    return category_id


async def _ensure_government_world_category(db: AsyncIOMotorDatabase) -> str:
    """Ensure the Government-page-only World category exists (not homepage `world`)."""

    existing = await db[CATEGORIES_COLLECTION].find_one({"slug": GOVERNMENT_WORLD_CATEGORY_SLUG})
    if existing is not None:
        await db[CATEGORIES_COLLECTION].update_one(
            {"_id": existing["_id"]},
            {
                "$set": {
                    "name": GOVERNMENT_WORLD_CATEGORY_NAME,
                    "description": GOVERNMENT_WORLD_CATEGORY_DESCRIPTION,
                },
            },
        )
        return str(existing["_id"])
    ...
    await db[CATEGORIES_COLLECTION].insert_one(
        {
            "_id": category_id,
            "name": GOVERNMENT_WORLD_CATEGORY_NAME,
            "slug": GOVERNMENT_WORLD_CATEGORY_SLUG,
            "parent_id": None,
            "description": GOVERNMENT_WORLD_CATEGORY_DESCRIPTION,
            "created_at": utc_now().isoformat(),
        },
    )
    logger.info("Created Government page World category %s", GOVERNMENT_WORLD_CATEGORY_SLUG)
    return category_id


async def _ensure_government_layout(db: AsyncIOMotorDatabase, *, market_id: str) -> dict[str, Any]:
    """Ensure an active market-level government layout exists (not a region clone)."""

    layout = await db[LAYOUTS_COLLECTION].find_one(
        {
            "page_name": GOVERNMENT_PAGE_NAME,
            "market_id": market_id,
    ...
    logger.info("Created government layout for market %s", market_id)
    return layout


async def _ensure_region_government_layout(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
) -> dict[str, Any]:
    """Ensure a region-owned government layout exists, cloning from market when needed.

    Args:
        db: Mongo database.
        market_id: Market document id.
        region_id: Region document id that must own the government board.

    Returns:
        Layout document for the region government page.

    Raises:
        ValidationError: When no market government layout exists to clone from.
    """
    ...
    await _ensure_government_layout(db, market_id=market_id)
    ...
    if layout_id is None:
        raise ValidationError(f"Unable to ensure government layout for region {region_id}")

    layout = await db[LAYOUTS_COLLECTION].find_one({"_id": layout_id})
    if layout is None:
        raise ValidationError(f"Government layout missing after ensure: {layout_id}")
    return layout


def _government_slot_query_rule(
    *,
    position_key: str,
    limit: int,
    existing: dict[str, Any] | None,
) -> dict[str, Any]:
    """Build query_rule for a government layout slot.

    Topic grids are always pin-only so configuration-added government start empty.
    ...
    if position_key not in PRESERVED_GOVERNMENT_PAGE_KEYS:
        return {"limit": limit}
...
async def _upsert_layout_slot(
    ...
    *,
    ...
    """Create or update one government layout slot without wiping existing pins."""
    ...
    fields = {
        "query_rule": _government_slot_query_rule(
...
async def _delete_obsolete_government_slots(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    keep_slot_ids: set[str],
) -> None:
    """Remove government-page slots that are no longer in the active list."""
...
async def _apply_government_slots_to_layout(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    ...
    """Upsert government slots onto one layout and delete obsolete rows.
    ...
    await _delete_obsolete_government_slots(db, layout_id=layout_id, keep_slot_ids=set(slot_ids))
    await db[LAYOUTS_COLLECTION].update_one(
        {"_id": layout_id},
...
async def _slot_spec_for_section(
    ...
    *,
    item: dict[str, str],
    order_index: int,
    government_category_id: str | None,
    live_category_id: str | None,
    world_category_id: str,
) -> dict[str, Any]:
    """Build one layout slot spec from a typed government page section item."""
    ...
    if section_type == SECTION_TYPE_HERO:
        return {
            ...
            "category_id": government_category_id,
            "limit": GOVERNMENT_HERO_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_TOP_STORIES:
        return {
            ...
            "category_id": government_category_id,
            "limit": GOVERNMENT_TOP_STORIES_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_LIVE:
        return {
            "position_key": slug,
            ...
            "limit": GOVERNMENT_LIVE_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_WORLD:
        return {
            ...
            "limit": GOVERNMENT_WORLD_ARTICLE_LIMIT,
        }
    ...
    category_id = await _ensure_government_topic_category(db, slug=slug, label=label)
    return {
        "position_key": slug,
        "order_index": order_index,
        "display_name": label,
        ...
        "limit": GOVERNMENT_SECTION_ARTICLE_LIMIT,
    }


async def sync_government_layout_slots(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    ...
    """Rebuild government page slots from an ordered typed section list.

    Expands legacy topic-only lists, then upserts slots in list order and deletes
    obsolete slots. Existing pins are preserved per ``position_key``.

    When ``region_id`` is set, updates only that region's government layout. Otherwise
    updates the market-level government layout only (does not mirror onto region boards).

    Args:
        db: Mongo database.
        market_id: Market document id.
        items: Ordered section rows (``section_type`` optional for legacy docs).
        region_id: Optional region document id for a state, county, or town government board.
    """

    resolved_items = expand_legacy_section_items(items)
    government_category_id = await _category_id_by_slug(db, PARENT_GOVERNMENT_CATEGORY_SLUG)
    live_category_id = await _category_id_by_slug(db, LIVE_CATEGORY_SLUG)
    world_category_id = await _ensure_government_world_category(db)
    if region_id:
        layout = await _ensure_region_government_layout(
            db,
            market_id=market_id,
            region_id=region_id,
        )
    else:
        layout = await _ensure_government_layout(db, market_id=market_id)
    ...
    for index, item in enumerate(resolved_items):
        slot_specs.append(
            await _slot_spec_for_section(
                ...
                item=item,
                order_index=index,
                government_category_id=government_category_id,
                live_category_id=live_category_id,
                world_category_id=world_category_id,
            ),
        )

    await _apply_government_slots_to_layout(
        db,
        layout_id=layout_id,
...
async def _resolve_region_government_items(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
    ...
    """Load or create government section items for one region.
...
async def ensure_region_government_sections(
    db: AsyncIOMotorDatabase,
    *,
    market_code: str,
    ...
    """Upsert government section lists and sync layouts for the given region codes.
    ...
    already auto-fill are not rewritten to pin-only topic slots.

    Args:
        db: Mongo database.
        market_code: Market short code that owns the regions (e.g. ``us``, ``pr``).
        region_codes: Ordered region codes to ensure.
        labels: Ordered topic display labels (typically PR topic labels).
    ...
    if market is None:
        raise ValidationError(
            f"{market_code.upper()} market not found; cannot seed government sections",
        )

    market_id = str(market["_id"])
    default_items = default_government_page_section_items(labels)
    now = utc_now().isoformat()
    ...
    for region_code in region_codes:
        ...
        if region is None:
            logger.warning("Skipping government sections; region missing: %s", region_code)
            continue
        region_id = str(region["_id"])
        items, created, should_sync = await _resolve_region_government_items(
            db,
            market_id=market_id,
            region_id=region_id,
        ...
        if created:
            created_count += 1
        if should_sync:
            await sync_government_layout_slots(
                db,
                market_id=market_id,
                items=items,
                region_id=region_id,
    ...
    logger.info(
        "Ensured %s government sections for %d regions (%d created, %d government)",
        market_code,
        len(ensured_codes),
        created_count,
        len(labels),
...
async def ensure_us_state_government_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert PR-shaped government section lists and sync layouts for every US state.
    ...
    return await ensure_region_government_sections(
        db,
        market_code="us",
...
async def ensure_florida_county_government_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert government section lists and sync layouts for every Florida county.
...
async def ensure_pr_town_government_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert government section lists and sync layouts for every Puerto Rico town.
...
async def ensure_geo_government_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Ensure independent government lists for US states, FL counties, and PR towns.
    ...
    states = await ensure_us_state_government_sections(db, labels=labels)
    counties = await ensure_florida_county_government_sections(db, labels=labels)
    towns = await ensure_pr_town_government_sections(db, labels=labels)
    return {
        "us_states": states,
