"""Shared helpers to sync health page section lists into layouts/slots."""
...
HEALTH_PAGE_NAME = "health"
HERO_POSITION_KEY = "hero"
US_FEATURED_POSITION_KEY = "us-featured"
LIVE_POSITION_KEY = "health"
PARENT_HEALTH_CATEGORY_SLUG = "finance"
PRESERVED_HEALTH_PAGE_KEYS = frozenset(
    {
        HERO_POSITION_KEY,
        US_FEATURED_POSITION_KEY,
        LIVE_POSITION_KEY,
        PARENT_HEALTH_CATEGORY_SLUG,
    },
)
...
def insert_legacy_health_ribbon_ads(items: list[dict[str, str]]) -> list[dict[str, str]]:
    """Insert ribbon rows matching the former health-page heuristic placements.
    ...
    for item in items:
        ...
        if section_type == SECTION_TYPE_LIVE:
            ...
            section_type == SECTION_TYPE_HEALTH
            and compact_index > 0
            and compact_index % HEALTH_SECTION_PAIR_SIZE == 0
        ):
            needs_before = True
        elif (
        ...
        if section_type == SECTION_TYPE_HERO:
            ...
            used_slugs.add(ribbon_slug)
            result.append(_ribbon_ad_item(ribbon_slug))
        if section_type == SECTION_TYPE_HEALTH:
            compact_index += 1
...
HEALTH_SECTION_ARTICLE_LIMIT = 12
HEALTH_HERO_ARTICLE_LIMIT = 12
HEALTH_TOP_STORIES_ARTICLE_LIMIT = 12
HEALTH_LIVE_ARTICLE_LIMIT = 20
LIVE_CATEGORY_SLUG = "health"
_SLUG_SAFE_RE = re.compile(r"[^a-z0-9]+")


def slugify_health_label(label: str) -> str:
    """Derive a URL-safe health slug from a display label.

    Args:
        label: Human-readable health name.
    ...
    if not normalized:
        raise ValidationError("Health label must contain letters or numbers")
    return normalized


def expand_legacy_health_section_items(items: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Normalize stored items and prepend fixed sections for legacy health-only lists.

    Args:
        items: Raw health_page_sections items (may omit ``section_type``).
    ...
    if has_typed:
        ...
        for item in items:
            section_type = str(item.get("section_type") or SECTION_TYPE_HEALTH).strip().lower()
            if section_type not in HEALTH_PAGE_SECTION_TYPES:
                section_type = SECTION_TYPE_HEALTH
            resolved.append(
                {
    ...
    return insert_legacy_health_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS] + health,
    )


def health_rows_from_labels(labels: list[str]) -> list[dict[str, str]]:
    """Build typed health rows from display labels.

    Args:
        labels: Ordered health display names.

    Returns:
        Health-typed section items with derived slugs.
    """
...
def default_health_page_section_items(labels: list[str]) -> list[dict[str, str]]:
    """Full default health page list: fixed bands plus health rows.

    Args:
        labels: Ordered health display names.

    Returns:
        Fixed sections followed by health rows, with legacy ad ribbons inserted.
    """

    return insert_legacy_health_ribbon_ads(
        [dict(row) for row in DEFAULT_FIXED_SECTION_ITEMS_WITHOUT_RIBBONS]
        + health_rows_from_labels(labels),
    )


async def _ensure_health_category(db: AsyncIOMotorDatabase, *, slug: str, label: str) -> str:
    """Ensure a global category exists for an health slug under parent Health."""

    parent = await db[CATEGORIES_COLLECTION].find_one({"slug": PARENT_HEALTH_CATEGORY_SLUG})
    parent_id = str(parent["_id"]) if parent else None
    if slug == PARENT_HEALTH_CATEGORY_SLUG and parent is not None:
    ...
    if existing is not None:
        await db[CATEGORIES_COLLECTION].update_one(
            {"_id": existing["_id"]},
            {
                "$set": {
                    "name": label,
                    "description": f"{label} health news.",
                    "parent_id": parent_id,
                },
            },
        )
    ...
    logger.info("Created health category %s", slug)
    return category_id


async def _ensure_health_layout(db: AsyncIOMotorDatabase, *, market_id: str) -> dict[str, Any]:
    """Ensure an active market-level health layout exists (not a region clone)."""

    layout = await db[LAYOUTS_COLLECTION].find_one(
        {
            "page_name": HEALTH_PAGE_NAME,
            "market_id": market_id,
    ...
    logger.info("Created health layout for market %s", market_id)
    return layout


async def _ensure_region_health_layout(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
) -> dict[str, Any]:
    """Ensure a region-owned health layout exists, cloning from market when needed.

    Args:
        db: Mongo database.
        market_id: Market document id.
        region_id: Region document id that must own the health board.

    Returns:
        Layout document for the region health page.

    Raises:
        ValidationError: When no market health layout exists to clone from.
    """
    ...
    await _ensure_health_layout(db, market_id=market_id)
    ...
    if layout_id is None:
        raise ValidationError(f"Unable to ensure health layout for region {region_id}")

    layout = await db[LAYOUTS_COLLECTION].find_one({"_id": layout_id})
    if layout is None:
        raise ValidationError(f"Health layout missing after ensure: {layout_id}")
    return layout


def _health_slot_query_rule(
    *,
    limit: int,
    existing: dict[str, Any] | None,
    category_id: str | None,
) -> dict[str, Any]:
    """Build query_rule so health slots auto-fill from their category.
...
async def _upsert_layout_slot(
    ...
    *,
    ...
    """Create or update one health layout slot without wiping existing pins."""

    existing = await db[SLOTS_COLLECTION].find_one(
        {"layout_id": layout_id, "position_key": position_key},
    )
    fields = {
        "query_rule": _health_slot_query_rule(
            limit=limit,
            existing=existing,
...
async def _delete_obsolete_health_slots(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    keep_slot_ids: set[str],
) -> None:
    """Remove health-page slots that are no longer in the active list."""
...
async def _apply_health_slots_to_layout(
    db: AsyncIOMotorDatabase,
    *,
    layout_id: str,
    ...
    """Upsert health slots onto one layout and delete obsolete rows.
    ...
    await _delete_obsolete_health_slots(db, layout_id=layout_id, keep_slot_ids=set(slot_ids))
    await db[LAYOUTS_COLLECTION].update_one(
        {"_id": layout_id},
...
async def _slot_spec_for_section(
    ...
    *,
    item: dict[str, str],
    order_index: int,
    health_category_id: str | None,
    live_category_id: str | None,
) -> dict[str, Any]:
    """Build one layout slot spec from a typed health page section item."""
    ...
    if section_type == SECTION_TYPE_HERO:
        return {
            ...
            "limit": HEALTH_HERO_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_TOP_STORIES:
        return {
            ...
            "category_id": health_category_id,
            "limit": HEALTH_TOP_STORIES_ARTICLE_LIMIT,
        }
    if section_type == SECTION_TYPE_LIVE:
        return {
            "position_key": slug,
            ...
            "limit": HEALTH_LIVE_ARTICLE_LIMIT,
        }
    ...
    category_id = await _ensure_health_category(db, slug=slug, label=label)
    return {
        "position_key": slug,
        "order_index": order_index,
        "display_name": label,
        ...
        "category_id": category_id,
        "limit": HEALTH_SECTION_ARTICLE_LIMIT,
    }


async def sync_health_layout_slots(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    ...
    """Rebuild health page slots from an ordered typed section list.

    Expands legacy health-only lists, then upserts slots in list order and deletes
    obsolete slots. Existing pins are preserved per ``position_key``.

    When ``region_id`` is set, updates only that region's health layout. Otherwise
    updates the market-level health layout only (does not mirror onto region boards).

    Args:
        db: Mongo database.
        market_id: Market document id.
        items: Ordered section rows (``section_type`` optional for legacy docs).
        region_id: Optional region document id for a state, county, or town health board.
    """

    resolved_items = expand_legacy_health_section_items(items)
    health_category_id = await _category_id_by_slug(db, PARENT_HEALTH_CATEGORY_SLUG)
    live_category_id = await _category_id_by_slug(db, LIVE_CATEGORY_SLUG)
    if region_id:
        layout = await _ensure_region_health_layout(
            db,
            market_id=market_id,
            region_id=region_id,
        )
    else:
        layout = await _ensure_health_layout(db, market_id=market_id)
    ...
    for index, item in enumerate(resolved_items):
        slot_specs.append(
            await _slot_spec_for_section(
                ...
                item=item,
                order_index=index,
                health_category_id=health_category_id,
                live_category_id=live_category_id,
            ),
        )

    await _apply_health_slots_to_layout(
        db,
        layout_id=layout_id,
...
async def _resolve_region_health_items(
    db: AsyncIOMotorDatabase,
    *,
    market_id: str,
    region_id: str,
    ...
    """Load or create health section items for one region.
...
async def ensure_region_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    market_code: str,
    ...
    """Upsert health section lists and sync layouts for the given region codes.
    ...
    already auto-fill are not rewritten to pin-only health slots.

    Args:
        db: Mongo database.
        market_code: Market short code that owns the regions (e.g. ``us``, ``pr``).
        region_codes: Ordered region codes to ensure.
        labels: Ordered health display labels (typically PR health labels).
    ...
    if market is None:
        raise ValidationError(
            f"{market_code.upper()} market not found; cannot seed health sections",
        )

    market_id = str(market["_id"])
    default_items = default_health_page_section_items(labels)
    now = utc_now().isoformat()
    ...
    for region_code in region_codes:
        ...
        if region is None:
            logger.warning("Skipping health sections; region missing: %s", region_code)
            continue
        region_id = str(region["_id"])
        items, created, should_sync = await _resolve_region_health_items(
            db,
            market_id=market_id,
            region_id=region_id,
        ...
        if created:
            created_count += 1
        if should_sync:
            await sync_health_layout_slots(
                db,
                market_id=market_id,
                items=items,
                region_id=region_id,
    ...
    logger.info(
        "Ensured %s health sections for %d regions (%d created, %d health)",
        market_code,
        len(ensured_codes),
        created_count,
        len(labels),
...
async def ensure_us_state_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert PR-shaped health section lists and sync layouts for every US state.
    ...
    return await ensure_region_health_sections(
        db,
        market_code="us",
...
async def ensure_florida_county_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert health section lists and sync layouts for every Florida county.
...
async def ensure_pr_town_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert health section lists and sync layouts for every Puerto Rico town.
...
async def ensure_country_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Upsert health lists for the US and Puerto Rico country boards.

    Configuration sends region ``us`` / ``pr`` when no state or town is selected.

    Args:
        db: Mongo database.
        labels: Ordered health display labels.

    Returns:
        Summary keyed by country code.
    """

    us_board = await ensure_region_health_sections(
        db,
        market_code="us",
        region_codes=("us",),
        labels=labels,
    )
    pr_board = await ensure_region_health_sections(
        db,
        market_code="pr",
        region_codes=("pr",),
        labels=labels,
...
async def ensure_geo_health_sections(
    db: AsyncIOMotorDatabase,
    *,
    labels: list[str],
) -> dict[str, Any]:
    """Ensure health lists for countries, US states, FL counties, and PR towns.
    ...
    countries = await ensure_country_health_sections(db, labels=labels)
    states = await ensure_us_state_health_sections(db, labels=labels)
    counties = await ensure_florida_county_health_sections(db, labels=labels)
    towns = await ensure_pr_town_health_sections(db, labels=labels)
    return {
        "countries": countries,
        "us_states": states,
